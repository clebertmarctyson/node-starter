...
program
  ...
  .action(async () => {
      ...
      if (folders) {
        ...
        folderNames.forEach((folderName, index) => {
          ...
          if (index === folderNames.length - 1) {
            spinner.success(
              chalk.green(`Folders ${folders} have been created successfully.`)
            );
          }
        });
      }
      ...
      if (files) {
        ...
        fileNames.forEach((fileName, index) => {
          ...
          if (index === fileNames.length - 1) {
            spinner.success(
              chalk.green(`Files ${files} have been created successfully.`)
            );
          }
        });
      }
    ...
    const initScript = `cd ${projectName} && ${packageManager} ${
      packageManager === "pnpm" ? "init" : "init -y"
    }`;
    const installScript = `cd ${projectName} && ${packageManager} ${
